...
namespace LessonA.DaySix
{
    internal class DictionaryDemo
    {
        ...
        public static void TestSortedListA()  //SortedList is the dictionary.
        {
        ...
        public static void TestSortedListOfEmp()
        {
            ...
            if (!empsortlist.ContainsKey(e1.ID))
                empsortlist.Add(e1.ID, e1);  //92(key),e1(Value)
            ...
            {
                Console.WriteLine(emp.Key + " : " + emp.Value.Name+emp.Value.ID);
            }
            Console.WriteLine("===================");
            var orderedlist = empsortlist.OrderBy(tkey => tkey.Value.Name);  //can be x => x.Value.Name
